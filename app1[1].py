...
# ---------------------- Load Reduced Data ----------------------
# Load compressed CSV
data = pd.read_csv("reduced_cleaned_data.csv.zip")

# Load reduced .npy feature file
final_features = np.load("reduced_final_features.npy")
...
ratings = data[['RecipeId', 'Rating']].copy()
ratings['userId'] = data['RecipeId']  # simulate since AuthorId is removed
ratings = ratings[['userId', 'RecipeId', 'rating']].dropna()
...
user_id = st.number_input("Enter User ID (any number):", min_value=1, step=1)
...
if veg:
    sub_pref = st.selectbox("Veg Preference", ["none", "paneer", "tofu", "mushroom"])
elif nonveg:
...
max_time = st.slider("Max Total Cook Time (minutes):", 5, 180, 30)
